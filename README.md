...
- run tests multiple times using repeat-each option  
  `npx playwright test --repeat-each=10` - retry option when running tests  
   `npx playwright test --retries=3` - retry option in playwright.config.ts  
   `retries: process.env.CI ? 2 : 2,` - retry option for test configuration (in code)  
   `test.describe.configure({ retries: 2 });` - add smart wait for page to fully load  
...
- Formatting: editor -> context menu -> Format Document -> <kbd>Shift</kbd> + <kbd>Alt</kbd> + <kbd>F</kbd>
